...
export default {
  ...
  methods:{
    ...
    save(){
      ...
      if (this.form.id){//更新
        request.put("/api/news/updata",this.form).then(res => {console.log(res)
        ...
        request.post("/api/news/save",this.form).then(res => {console.log(res)
          if (res.code === '0') {//res.code是放回结果的一些性质,0就是成功
    ...
    load(){
      request.get("/api/news/findPage",{params: {
    ...
    handleDelete(id){
      console.log(id)
      request.delete("/api/news/delNews/"+id).then(res=>{
        if (res.code === '0') {//res.code是放回结果的一些性质,0就是成功
