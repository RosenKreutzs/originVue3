...
export default {
  ...
  methods:{
    ...
    save(){
      if (this.form.id){//更新
        request.put("/api/book/updata",this.form).then(res => {console.log(res)
        ...
        request.post("/api/book/save",this.form).then(res => {console.log(res)
          if (res.code === '0') {//res.code是放回结果的一些性质,0就是成功
    ...
    load(){
      request.get("/api/book/findPage",{params: {
    ...
    handleDelete(id){
      console.log(id)
      request.delete("/api/book/delBook/"+id).then(res=>{
        if (res.code === '0') {//res.code是放回结果的一些性质,0就是成功
